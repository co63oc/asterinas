...
impl Inode {
    ...
    pub(in crate::fs::fs_impls::ext2) fn rename(
        ...
        match mode {
            RenameMode::Exchange => {
                let replaced = replaced_inode
                    .as_ref()
                    .expect("exchange mode requires a replaced inode");
                self.apply_exchange_mutations(
                    &mut guards,
                    target,
                    old_name,
                    &old_inode,
    ...
    fn apply_exchange_mutations(
        &self,
        guards: &mut MultiInodeInnerGuards,
        target: &Inode,
        old_name: &str,
        old_inode: &Inode,
        replaced_inode: &Inode,
        new_name: &str,
    ) -> Result<()> {
        let old_is_dir = old_inode.type_ == InodeType::Dir;
        ...
        let replaced_is_dir = replaced_inode.type_ == InodeType::Dir;
        let replaced_file_type = DirEntryFileType::from(replaced_inode.type_);
        let replaced_ino = replaced_inode.ino();
        ...
        if is_same_dir {
            ...
            if replaced_is_dir {
                let replaced_inner = guards.inner_mut(replaced_inode.ino());
